...
final class Ingest
{
    ...
    public static function handle(PDO $db, Config $cfg, array $q, array $server): void
    {
        ...
        // NEW: phone type/model identifier (best-effort)
        $model = null;
        if (isset($q['model'])) {
            $model = trim((string) $q['model']);
        } elseif (isset($q['phone'])) {
            $model = trim((string) $q['phone']);
        } elseif (isset($q['device'])) {
            $model = trim((string) $q['device']);
        }
        if ($model === '') {
            $model = null;
        }
        // keep it bounded
        if ($model !== null && strlen($model) > 64) {
        ...
        $stmt = $db->prepare("
            INSERT INTO yealink_events
                (received_at, ext, call_id, event, local_uri, remote_uri, display_local, display_remote, phone_model, source_ip)
            VALUES
                (NOW(3), :ext, :call_id, :event, :local_uri, :remote_uri, :display_local, :display_remote, :phone_model, :source_ip)
        ");
        $stmt->execute([
            ':ext'            => $ext,
            ':call_id'        => $callId,
        ...
        $sql = "
            INSERT INTO yealink_calls
                (ext, call_id, first_seen_at, {$timeCol},
                 local_uri, remote_uri, display_local, display_remote, phone_model,
            ...
            ON DUPLICATE KEY UPDATE
                ...
                phone_model    = COALESCE(phone_model,    VALUES(phone_model)),
